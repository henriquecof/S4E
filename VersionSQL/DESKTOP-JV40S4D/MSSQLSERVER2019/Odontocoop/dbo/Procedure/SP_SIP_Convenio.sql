/****** Object:  Procedure [dbo].[SP_SIP_Convenio]    Committed by VersionSQL https://www.versionsql.com ******/

create Procedure [dbo].[SP_SIP_Convenio]
...
Begin
	...
	delete ans_sip_convenios where ano_trimestre = @ano_trimeste and tipo_empresa = @tp_empresa and IDConvenio = @Convenio  --and uf = @cd_uf 
 ...
 -----------------------------------------------------------------------------------------  
   ...
   Select top 1 @cd_planoans = cd_ans from classificacao_ans where tp_empresa = @tp_empresa and idConvenio = @Convenio
   ...
   select distinct ISNULL(t2.ufid,isnull(t3.ufid,@ufp)) 
     from pagamento_dentista_lancamento as t1 
	         inner join pagamento_dentista as t4 on t1.cd_pgto_dentista_lanc = t4.cd_pgto_dentista_lanc
			 inner join Consultas as t5 on t4.cd_sequencial = t5.nr_numero_lote
			 inner join DEPENDENTES as t6 on t5.cd_sequencial_dep = t6.CD_SEQUENCIAL
			 inner join planos as t7 on t6.cd_plano = t7.cd_plano 
			 inner join CLASSIFICACAO_ANS as t8 on t7.cd_classificacao = t8.cd_classificacao
   ...
   While @qtde_curr<=@qtde_repeticao
	Begin
	  ...
	  select convert(varchar,year(dateadd(day,-1,DATEADD(month,@qtde_curr,@dt_ref)))) + right('00'+convert(varchar,month(dateadd(day,-1,DATEADD(month,@qtde_curr,@dt_ref)))),2), 
	         isnull(COUNT(0),0) ,
			 isnull(sum(case when dbo.FS_Idade(d.dt_nascimento,DATEADD(month,@qtde_curr,@dt_ref))>=12 then 1 else 0 end),0),
			 isnull(sum(case when dbo.FS_Idade(d.dt_nascimento,DATEADD(month,@qtde_curr,@dt_ref))>=12 then 0 else 1 end),0)
		from ASSOCIADOS as a inner join DEPENDENTES as d on a.cd_associado = d.cd_associado 
			    ...
			    inner join planos as t7 on d.cd_plano = t7.cd_plano 
			    inner join CLASSIFICACAO_ANS as t8 on t7.cd_classificacao = t8.cd_classificacao
	   ...
	   where d.dt_assinaturaContrato  < DATEADD(month,@qtde_curr,@dt_ref)
		 and (sh.fl_incluir_ans = 1
		    or 
			  (sh.fl_incluir_ans = 0 and h.dt_situacao >= DATEADD(month,@qtde_curr,@dt_ref) )
			 )
         and t8.tp_empresa=@tp_empresa
					  ...
					  from categoriaANS_Servico as T2, Consultas as T3, DEPENDENTES as T4,
						   ...
						   (  select t1.* 
								from pagamento_dentista_lancamento as t1 
									...
									left join funcionario as t2 on t1.cd_funcionario = t2.cd_funcionario
									left join filial as t3 on t1.cd_filial = t3.cd_filial
							  where t1.dt_conhecimento >= @dt_ref 
							    and t1.dt_conhecimento < DATEADD(month,3,@dt_ref) 	
								and t8.idConvenio = @Convenio
								and  ISNULL(t2.ufid,isnull(t3.ufid,@ufp))  = @cd_uf       
						   ) as pdl, 
						   SERVICO T5
					 where 
				...
				DECLARE cursor_gera_sip CURSOR FOR 
				 select top 100 cd_categoria_ans , padrao_idade, ud_inicial, ud_final , 1-- isnull(peso,1)
				  ...
				  begin  -- 2.2        
					 ...
					 set @sql = 'insert into ans_sip_convenios (ano_trimestre,tipo_empresa,cd_categoria_ans,qt_beneficiarios,vl_despesa,qt_eventos,uf,ano_trimestre_execucao,Idconvenio)
						Select '+CONVERT(varchar(4),@ano)+right('00'+CONVERT(varchar(2),@trimestre),2) +','+
						  ...
						  from categoriaANS_Servico as T2, Consultas as T3, DEPENDENTES as T4,
							   ...
							   (select t1.* 
								  from pagamento_dentista_lancamento as t1 
										...
										left join funcionario as t2 on t1.cd_funcionario = t2.cd_funcionario
										left join filial as t3 on t1.cd_filial = t3.cd_filial
								  where T1.dt_conhecimento >= ''' + convert(varchar(10),@dt_ref,101) + ''' 
								    and T1.dt_conhecimento < DATEADD(month,3,''' + convert(varchar(10),@dt_ref,101) + ''') 
									and t8.idConvenio = ' + convert(varchar(10),@Convenio) + ' 
									and ISNULL(t2.ufid,isnull(t3.ufid,'+convert(varchar(4),@ufp)+')) = '+convert(varchar(4),@cd_uf)+'		 
							   ) as pdl, 
							   SERVICO T5
						 where t2.cd_categoria_ans = ' + convert(varchar(10),@cd_categoria_ans) + ' and
							...
							--	   T3.vl_glosa>0
							   ...
							   cans.idconvenio = ' + convert(varchar(10),@Convenio) + ' 
							   '
	   ...
	   Deallocate cursor_uf		
		...
		update ans_sip_convenios
		 ...
		 from (select distinct ano_trimestre, tipo_empresa, cd_categoria_ans, qt_beneficiarios 
				 from ans_sip_convenios 
				where ano_trimestre <> CONVERT(varchar(4),@ano)+right('00'+CONVERT(varchar(2),@trimestre),2)
				  and ano_trimestre = ano_trimestre_execucao
				  and IdConvenio = @Convenio 
				  and cd_arquivo in (select max(cd_arquivo)
									   from ans_sip_convenios 
									  where ano_trimestre = ano_trimestre_execucao and IdConvenio = @Convenio 
									  group by ano_trimestre, tipo_empresa, cd_categoria_ans
									 ) 
			   ) as s1
		where ans_sip_convenios.ano_trimestre = CONVERT(varchar(4),@ano)+right('00'+CONVERT(varchar(2),@trimestre),2) 
		  and ans_sip_convenios.ano_trimestre_execucao <> ans_sip_convenios.ano_trimestre 
		  and ans_sip_convenios.tipo_empresa = s1.tipo_empresa 
		  and ans_sip_convenios.cd_categoria_ans=s1.cd_categoria_ans
		 -- and ANS_SIP.uf=s1.uf
		  and ans_sip_convenios.ano_trimestre_execucao=s1.ano_trimestre
		  and ans_sip_convenios.IdConvenio = @Convenio 
		...
		select ano_trimestre_execucao, tipo_empresa, uf 
		  ...
		  WHERE ANO_TRIMESTRE= @ano_trimeste
			and qt_eventos=0 
			and vl_despesa=0
			and cd_categoria_ans=1
			and IdConvenio = @Convenio) as x 
		 where ans_sip_convenios.ano_trimestre=@ano_trimeste
		   and ans_sip_convenios.ano_trimestre_execucao = x.ano_trimestre_execucao
		   and ans_sip_convenios.tipo_empresa=x.tipo_empresa 
		   and ans_sip_convenios.uf = x.uf 
		   and IdConvenio = @Convenio
End 
