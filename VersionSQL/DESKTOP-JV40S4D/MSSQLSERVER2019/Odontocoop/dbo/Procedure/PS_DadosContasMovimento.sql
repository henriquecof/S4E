/****** Object:  Procedure [dbo].[PS_DadosContasMovimento]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[PS_DadosContasMovimento]
        (
...
AS	
 Begin    
   ...
   -----------------------------------------------------------------------------
       ...
       Declare Dados_Cursor  Cursor For 
		...
		where  t1.sequencial_conta        = t3.sequencial_conta And         
			  ...
			  ((t2.data_pagamento Between @Data_Inicial And @Data_Final) Or         
               ...
               T1.Sequencial_Conta In 
						   (Select Sequencial_Conta 
								From TB_Conta 
								Where Sequencial_Conta_Destino is not null) And                
				T4.Codigo_ContaMae >= @ContaMae And   
			    T4.Codigo_ContaMae <= @WL_ContaMaeFinal And 
				T1.Codigo_Conta >= @Conta And   
				T1.Codigo_Conta <= @WL_ContaFinal
		  Order By 1 Asc 
      ...
      While (@@fetch_status  <> -1)
         Begin
              ...
              Where   T1.Sequencial_Conta      = T3.Sequencial_Conta And 
                      ...
                      T1.Sequencial_ContaMae   = T4.Sequencial_ContaMae And
                      --T1.Codigo_Conta          = @WL_Conta_Origem And
                      --((T2.data_pagamento Between @Data_Inicial And  @Data_Final) Or 
				       --(T2.data_documento Between @Data_Inicial And  @Data_Final And T2.data_pagamento is null) or         
                       --T2.Valor_Lancamento     = @WL_Valor_LancamentoOrigem Or 
                       --T2.Valor_Previsto       = @WL_Valor_PrevistoOrigem Or
    ...
    ----------------------------------------------------------------------------
    -- 2º Passo : Sem contas de contra-partida  - Primeiro só os recebimentos    
    ----------------------------------------------------------------------------     
     Declare Dados_Cursor  Cursor For 
	  Select   T1.Codigo_Conta,
						T1.Descricao_Conta, 
		...
		Where  t1.sequencial_conta        = t3.sequencial_conta And         
			  ...
			  ((t2.data_pagamento Between @Data_Inicial And @Data_Final) Or         
			   (t2.data_documento Between @Data_Inicial And @Data_Final And t2.data_pagamento is null)) And       
                --Substring(t1.Codigo_Conta,3,2) Not In ('35','46','47','66') And               
                --Substring(t1.Codigo_Conta,3,2) Not In ('47','66') And               
                T1.Sequencial_Conta Not In 
				  (Select Sequencial_Conta_Destino 
						From TB_Conta 
						Where Sequencial_Conta_Destino is not null) And                
                T1.Sequencial_Conta Not In 
				  (Select Sequencial_Conta
